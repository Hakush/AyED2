...
int main(int argc, char *argv[]) {
    ...
    fprintf(stdout,"\nLa minima temperatura minima historica es: %d\n", min_tempMin_hist(array));


    int output[YEARS];
    max_tempMax_eachY(array, output);
    for (int i = 0; i < YEARS; ++i)
    {
        fprintf(stdout,"Max temp in the year %u is : %d\n", (i + FST_YEAR), output[i]);
    }

    unsigned int output2[YEARS];
    month_with_max_prec_eachY(array, output2);
    char* months[] = {"january","february","march","april","may","june","july","august","september","october","november","december"};

    for (int year = 0; year < YEARS; ++year)
    {
        fprintf(stdout,"Month with the max rainfall in the year %u is : %s\n", (year + FST_YEAR), months[output2[year]]);
    }
